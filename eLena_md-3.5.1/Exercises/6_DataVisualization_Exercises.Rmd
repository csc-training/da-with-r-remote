...
**1.2** Then download a version of the `surveys` data set that we worked with yesterday. If you already have the data ready, this step can be skipped.

```r
download.file(url = "https://tinyurl.com/surveyscomplete",
 destfile = "data_output/surveys_complete.csv")

# note that we're downloading this into the data_output folder!
```

**1.3** Import the data into a tibble called `surveys_complete`, using `read_csv()`.

**1.4** Using `ggplot2` and the `surveys_complete` data set, create a scatter plot of `weight` over `species_id` with the plot types shown in different colors. What do you think - is this a good way to show these type of data? If not, how would you go about it instead?

**1.5** Try setting up a plot template using the code you just wrote for creating the scatter plot above.
