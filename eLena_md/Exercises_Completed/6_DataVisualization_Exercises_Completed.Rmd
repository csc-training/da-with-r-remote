...
**8.3** We can use `ggsave()` to save the plots. Try saving a single plot first. Instead of png, save the plot as a pdf file.
...
**8.4** Next we could save a `grid.arrange()` plot. First use `grid.arrange()` to combine plots 1 and 2, then use `ggsave()` to save the result as a pdf file (use a DPI value of 100).
