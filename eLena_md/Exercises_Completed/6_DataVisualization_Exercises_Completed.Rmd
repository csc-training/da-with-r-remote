...
`r r2symbols::sym("star-solid")` **1.2** Then import a version of the `surveys` data set that we worked with previously (`surveys_complete.csv`). Assign it to a tibble called `surveys_complete`, using `read_csv()`. We can use a version of the data located in the `/shared` folder.

```r
surveys_complete <- read_csv("/home/rstudio/shared/surveys_complete.csv")
```

`r r2symbols::sym("star-solid")` **1.3** Using `ggplot2` and the `surveys_complete` data set, create a scatter plot with `weight` on the y axis and `species_id` on the x axis, with the plot types shown in different colors. What do you think - is this a good way to show these type of data? If not, how would you go about it instead?
...
```r
ggsave("fig_output/plot1.pdf", my_plot, 
width = 15, height = 10)
```

`r r2symbols::sym("star-solid")` **8.4** Next we could save a `grid.arrange()` plot. First use `grid.arrange()` to combine plots 1 and 2, then use `ggsave()` to save the result as a pdf file (use a DPI value of 100).
...
ggsave("fig_output/comboplot.pdf", plots_arranged, 
width = 10, dpi = 100)
```
