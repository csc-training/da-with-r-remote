...
**1.2** `r r2symbols::sym("star-solid")` Then import a version of the `surveys` data set that we worked with previously (`surveys_complete.csv`). Assign it to a tibble called `surveys_complete`, using `read_csv()`. We can use a version of the data located in the `/shared` folder.

```r
surveys_complete <- read_csv("/home/rstudio/shared/surveys_complete.csv")
```

**1.3** `r r2symbols::sym("star-solid")`  Using `ggplot2` and the `surveys_complete` data set, create a scatter plot with `weight` on the y axis and `species_id` on the x axis, with the plot types shown in different colors. What do you think - is this a good way to show these type of data? If not, how would you go about it instead?
...
**8.3** `r r2symbols::sym("star-solid")` We can use `ggsave()` to save the plots. Try saving a single plot first. Instead of png, save the plot as a pdf file.
...
**8.4** `r r2symbols::sym("star-solid")` Next we could save a `grid.arrange()` plot. First use `grid.arrange()` to combine plots 1 and 2, then use `ggsave()` to save the result as a pdf file (use a DPI value of 100).
