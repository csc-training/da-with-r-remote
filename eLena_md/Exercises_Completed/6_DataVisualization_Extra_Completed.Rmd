...
# Using geom_line()
ggplot(data = plotdata,
       aes(x = week, y = cumulative_sum))+
  geom_line(size=1, aes(color = region))

# Using geom_col()
ggplot(data = plotdata,
       aes(x = week, y = cumulative_sum))+
  geom_col(size=1, aes(fill = region))
```
...
ggplot(data = plotdata %>%
         filter(region %in% c("Lappi Hospital District",
                              "Kainuu Hospital District",
                              "Helsinki and Uusimaa Hospital District") & cumulative_sum > 0),
       aes(x = week, y = cumulative_sum)) +
  geom_line(size=1, aes(color = region)) +
  scale_y_log10() +
...
If you don't remember how to save and export your data from RStudio Server, follow the instructions set out in the [Starting with data exercise sheet](https://e-learn.csc.fi/mod/resource/view.php?id=574) (steps 7.3 and 7.4).

That's it in terms of exercises! As this is the last part of the course, we will not go through the solutions in a Zoom session. However, if you're unsure about how to solve some of the problems above, you can have a look at the completed exercise sheets on eLena.

Thanks for joining the course and we hope you've found the contents useful!