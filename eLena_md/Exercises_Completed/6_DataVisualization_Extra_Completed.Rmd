...
``` r
ggplot(data = counts_for_plotting,
       aes(x = month, y = count)) +
  geom_col()
...
**2.3** Next, let's combine the above and make a line plot that shows month on the x axis, counts on the y axis, and the lines are coloured by region (`bio_province`).  Important: set both `color = bio_province` and group = `bio_province` in side the `aes()` function:   
`aes(x = ... , y = ... , colour = bio_province, group = bio_province)`. Additional hint: use `geom_line()`.

``` r
ggplot(data = counts_for_plotting,
       aes(x = month, y = count, color = bio_province, group = bio_province)) +
  geom_line()
  
# or 

ggplot(data = counts_for_plotting,
       aes(x = month, y = n, color = bio_province, group = bio_province)) +
  geom_line()
```
