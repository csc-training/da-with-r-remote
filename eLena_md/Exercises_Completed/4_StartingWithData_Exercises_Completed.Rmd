...
**6.3** The car colour labels contain accidental whitespaces. To fix this, try importing the data using the `strip.white` argument. 

NOTE - this argument must be accompanied by the `sep` argument, by which we indicate the type of delimiter in the file (the comma for most .csv files). In other words, we need to add `sep = ','` to the import command. 

Use `unique()` to check if it worked! Hint: you can refer to a specific column in a data frame with `dataframe$column`. 
