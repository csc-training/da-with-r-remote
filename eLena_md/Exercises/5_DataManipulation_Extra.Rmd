...
**2.1** Before getting started on the exercises for data manipulation, let's make a copy of the data to keep original data intact. Next, we will simplify the formatting of dates and remove double entries for regions (`bio_province`). We will also remove observations with dates in winter because we don't expect swallows to occur then. Copy-paste these lines to your R script and run them as they are.
...
**3.2** The column `bio_province` shows the region of the observation. Let's calculate the number of swallows observations (rows in the data) by region. Save the result in a data frame called `by_region`. 
