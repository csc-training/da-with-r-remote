...
This section shows how the data used in the following exercises was downloaded from FinBIF. **If you are carrying out these exercises for the addition 0.5 credits, it is recommended to use the pre-downloaded data and start from step 2.** 
...
First, let's load the `tidyverse` R package and then the pre-downloaded data into R. It is stored in the `shared` folder of our course RStudio environment in Noppe. Because the data are saved as an R object in an .rds file, we don't need an additional import command here. Now we use the command `readRDS` to read in the saved data.  
...
**3.2** The column `bio_province` shows the region of the observation. Let's count the number of swallows observations (rows in the data) by region. Save the result in an object called `by_region`. 
