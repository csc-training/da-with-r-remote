...
**Note:** If you haven't followed the previous exercises, you will need to download the data first: 

```r
download.file(url="https://tinyurl.com/portaljoined",
              destfile = "data/portal_data_joined.csv")

# note: you also need a 'data' folder for the above to work!
```        
...
#### **Block 5: spreading and gathering**

This topic is quite time-consuming, so chances are that you are looking at this section independently.

**5.1** Spread the `surveys` data frame with `year` as columns, `plot_id` as rows, and the number of genera per plot as the values. You will need to summarize before reshaping, and use the function `n_distinct()` to get the number of unique genera within a particular chunk of data.

**5.2** Now take that data frame and `gather()` it again, so each row is a unique `plot_id` by `year` combination.
...
**5.4** With this new data set, calculate the average of each `measurement` in each `year` for each different `plot_type`. Then `spread()` them into a data set with a column for `hindfoot_length` and `weight`. *Hint*: You only need to specify the key and value columns for `spread()`.
...
**6.4** Save the data as a CSV file in our `data_output` folder.

```r
write_csv(surveys_complete, path = "data_output/surveys_complete.csv")
```

If you run into troubles here, don't worry. When we continue with data visualization exercises tomorrow, we can download a ready version of the data from the web (isn't the Internet wonderful?).