...
**4.1** First assign the gender data from `surveys` to a new object and convers it to a factor. You probably also remember there are some measurements where the gender information is either missing or undetermined, let's call those "undetermined".
...
**6.2** Have a look at the structure of `carSpeeds` using `str()`. Then try re-importing the data with the header argument set to false: `header = FALSE`. How does the new data set differ from the previous one? 
...
**7.2** Export the car speeds data. The `write.csv()` function requires a minimum of two arguments, the data to be saved and the name of the output file. The output file name should be in quotation marks (because it is a character string and not an object in our R environment).
...
**7.5** Save your R script (File --> Save as...).

**7.6** Export your data from RStudio Server.
