...
```
The line in the output "`summarise()` regrouping output by 'sex' (override with `.groups` argument)" is not an error, but a recently introduced feature in the `dplyr` package. We won't go into it in detail, but for information: `dplyr` now comes with extra features for tweaking the output of split-apply-combine steps like the one above. This could be useful e.g. when transforming your data.
