...
**Note:** When you start creating more R code, using Git / Github is a great way to keep track of your code and work effectively with others. While we won't cover Git / GitHub lessons in this workshop, feel free to explore the materials below in your own time!

- [Software Carpentry lesson on maintaining version control over your data using Git](http://swcarpentry.github.io/git-novice/)
- [Coderefinery Git lesson](https://coderefinery.github.io/git-intro/)
- [RStudio-focused introduction to Git for R users](https://happygitwithr.com/) 
...
Use Save As and choose CSV as the file format (click the links to see examples as images): File -> [Save As](https://github.com/csc-training/da-with-r-remote/blob/master/eLena_md/LectureMaterials/Images/excel_save_as.png?raw=true){target="_blank"} -> [File Format](https://github.com/csc-training/da-with-r-remote/blob/master/eLena_md/LectureMaterials/Images/excel_file_format.png?raw=true){target="_blank"}: [Comma-separated values (.csv)](https://github.com/csc-training/da-with-r-remote/blob/master/eLena_md/LectureMaterials/Images/excel_file_name.png?raw=true){target="_blank"}. 
...
The default delimiter of the `read.csv()` function is a comma, but you can use other delimiters by supplying the ‘sep’ argument to the function. For example, typing `sep = ';'` allows a semi-colon separated file to be correctly imported (see `?read.csv()` for more information). Note that an alternative handy way to import a semi-colon separated file is to use the function `read.csv2()` (see `?read.csv()` for more information).
...
Before we carry on working with the survey data, let's have a closer look at some of the arguments in `read.csv`. First we will load another data set into R, this time containing information on the speeds at which cars of different colors were clocked in 45 mph zones in different US states. 
...
After altering our cars data set, we would now like to save the output. For this, we can use the `write.csv` function call (or `write.csv2`for semi-colon separated data). As with `read.csv`, there are several arguments to this function, a few of which are particularly important for how the data are exported. Let’s explore these now.
...
#### 5. Loading and saving R scripts
