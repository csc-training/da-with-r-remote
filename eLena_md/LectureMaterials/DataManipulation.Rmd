...
- Use `summarize`, `.by`, and `count` to split a data frame into groups of observations, apply summary statistics for each group, and then combine the results.
...
One example of the advantages of `tidyverse` is that when working with large CSV files, importing data with `read_csv` of `tidyverse` is faster than with `read.csv` that we used earlier.
...
Note: in R versions <4.0, converting strings to factors was the default behaviour and had to be prevented when importing data with `read.csv` by adding the argument `stringsAsFactors = FALSE`.
...
The last option, **pipes**, lets you take the output of one function and send it directly to the next. This is useful when you need to do many things to the same dataset. Pipes in R look like `|>` (often called the **native pipe operator or base R pipe**) or `%>%` (the **magrittr pipe** because it comes from the `magrittr` package, installed automatically with `dplyr`). For most purposes both pipes work the same way, but it is better to pick one and use it consistently. In the course materials, we will use the native pipe `|>`. 
...
Here is one example where tibbles and data frames work differently. If you read in the `surveys` data set with `read.csv` instead of the
tidyverse `read_csv`, `print(n = 15)` won't work. Instead, you can use the `head()` function we already used earlier:
