...
The scatter plot resulting from the exercise should look like this:
...
Choosing colors in R is a rather vast topic of its own and we won't be covering that in much detail here. However, here are some materials you may wish to check out in your own time!  
...
meanwt <- surveys_complete |>
           summarize(mean_weight = mean(weight), .by = species_id)
```
...
Faceting is a great tool for splitting one plot into multiple plots, but sometimes you may want to produce a single figure that contains multiple plots using different variables or even different data frames. Several R packages are allow faceting to create publication-ready multipanel figures, such as `gridExtra`, [patchwork](https://patchwork.data-imaginist.com/) and [cowplot](https://wilkelab.org/cowplot/index.html). Here, we use `gridExtra`. The `gridExtra` package allows us to combine separate ggplots into a single figure using `grid.arrange()`:
