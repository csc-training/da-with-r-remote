...
R is a programming language for handling, analysing and visualizing data. It is freely downloadable and works on all major platforms, including Windows, macOS and Linux. At its core, R code is made of text-based commands that can be run from a command prompt and without a graphical user interface. R can also be run via an open-source interface such as RStudio (see [RStudio web site](https://posit.co/products/open-source/rstudio/?sid=1)). RStudio provides a built in-editor and offers many advantages including ease of use, integration with version control and project management. 
...
#### 2. Accessing the RStudio teaching environment on noppe.csc.fi

To launch your own instance of R:

1. Go to [noppe.csc.fi](https://noppe.csc.fi/#/) and log in using the Haka or Virtu authentication service or your CSC account. If you are using a Noppe guest account, use the "special login" link.
2. In the top right corner, click "Join workspace" and enter the code provided by the instructors.
3. You should see a workspace with an RStudio environment called "RStudio for Data analysis with R, Sept 2026". Later you can find it under "My workspaces". 
...
3. Hit `Ctrl + Return` in Windows or Linux or `⌘ + Return` on macOS. 
